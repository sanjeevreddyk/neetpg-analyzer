...
async function initDatabase() {
  try {
    ...
    // Migration to add Gemini_Enriched to QuestionBank if not present
    try {
      await dbQuery.run(`ALTER TABLE QuestionBank ADD COLUMN Gemini_Enriched BOOLEAN DEFAULT 0;`);
      console.log('Database migration: Added Gemini_Enriched column to QuestionBank table.');
    } catch (e) {
      // Ignore error if column already exists
    }
